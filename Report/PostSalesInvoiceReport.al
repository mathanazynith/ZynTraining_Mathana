report 50127 "Posted Sales Invoice Report"
{
    ...
    dataset
    {
        dataitem(InvHeader; "Sales Invoice Header")
        {
            ...
            dataitem(ExtBegin; "ZYN_Extended Table")
            {
                ...
                DataItemTableView =
                    sorting("Line No.")
                    where(
                        "Text Code Type" = const("ZYN_Text Code Enum"::"Begin Text Code "),
                        Text = filter(<> '')
                    );
            ...
            dataitem(ExtEnd; "ZYN_Extended Table")
            {
                DataItemLink = "Document No." = field("No.");
                ...
                DataItemTableView =
                    sorting("Line No.")
                    where(
                        "Text Code Type" = const("ZYN_Text Code Enum"::"End Text Code"),
                        Text = filter(<> '')
                    );
